...
The `ggplot2` package is another highly recommended graphics system, but we do not (yet) have a version of this tutorial using `ggplot2` commands. If you already know `ggplot2` (or wish to learn!), an interim solution is to challenge yourself to make the plots you see below using `ggplot2`.
...
Somtimes, it can help to add jitter, a small bit of meaningless noise, in the horizontal position.
...
The full `photoRec` dataset has `r ncol(prDat)` samples and `r nrow(prDat)` probesets. Choose 2 ... or 20 ... or 200 random probesets/genes and look for gene expression differences between the two genotypes, wild type versus knockout. Make use of the graphing techniques discussed this week such as scatter plots, data heatmaps, correlation heatmaps, etc. Share questions, success, failure on the Google group.
