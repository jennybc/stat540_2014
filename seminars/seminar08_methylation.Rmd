...
> This seminar is inspired by a STAT 540 project in 2013: *Analysis of Gene Expression Omnibus Leukaemia Data from the Illumina HumanMethylation450 Array* by Alice Zhu, Rachel Edgar, Shaun Jackman and Nick Fishbane.    
...
system.time(GSE42865 <- getGEO('GSE42865')) # took ~2 mins for JB
...
## TO DO: better title
## these are average beta values, right?
## is it for 2 arrays or for two conditions or experiments or ???
```
...
```
```{r postNorm, echo = F}
## TO DO: I don't like this hard-wiring of column numbers. There has to be a
## better, more programmatic way to do this.
dat.probeMeans <- c(rowMeans(beta.norm[,1:33], na.rm = TRUE),
                    rowMeans(beta.norm[,34:42], na.rm = TRUE)) 
plotNorm <-
...
## TO DO: replace these hard-wired values for `times` below
col <- c(rep("darkgoldenrod1", times = 29), rep("forestgreen", times = 13))
## TO DO: we should be saving old pars and then restoring later
par(mai = rep(0.5, 4))
...
## JB gets Error in plot.new() : figure margins too large
